...
import io.github.joyk0117.paperworknavigator.common.cleanUpMediapipeTaskErrorMessage
...
import io.github.joyk0117.paperworknavigator.data.DEFAULT_VISION_ACCELERATOR
...
object LlmChatModelHelper : LlmModelHelper {
  ...
  override fun initialize(
    ...
    Log.d(TAG, "initialize: model=${model.name} taskId=$taskId maxNumTokens=$maxTokens")
    ...
    try {
      ...
      val nativeHeapMb = android.os.Debug.getNativeHeapAllocatedSize() / 1_048_576
      val vmHeapMb = Runtime.getRuntime().let { (it.totalMemory() - it.freeMemory()) / 1_048_576 }
      Log.i(TAG, "initialize done: model=${model.name} maxNumTokens=$maxTokens nativeHeap=${nativeHeapMb}MB vmHeap=${vmHeapMb}MB")
    } catch (e: Exception) {
