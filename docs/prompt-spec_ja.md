...
> **maxOutputTokens について**: LiteRT-LM 0.10.0 の Kotlin API（`SamplerConfig`・`ConversationConfig`）にはトークン上限を per-call で設定するパラメータが存在しない。安全網としての `maxOutputTokens` はプロンプト制約（`Keep each response under 3 sentences.`）で代替する。
...
| システムプロンプトに3文制限が含まれる | `mf07SystemPrompt()` の出力に `"Keep each response under 3 sentences."` が含まれる | 自動（TC-07-10） |
| 典型的な質問への応答が3文以内 | 「締め切りはいつか」「何を持参するか」等の質問に対して3文以内で返答される | 手動確認（TC-07-11） |
