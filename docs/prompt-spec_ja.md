...
> バージョン: 0.3.2
> 作成日: 2026-05-07
> 最終更新: 2026-05-19（MF-01c §2.1「画像確認必須」制約を緩和し、固有名詞を除く高確信度の言語知識による補正を許可・§2.4 に補正可否の境界例を追加）
...
**補正してよい例（語彙として存在しない語 / 明白なスペルミス）:**
```
CORRECT: 令利7年|令和7年
CORRECT: administraion|administration
```

**補正してはいけない例（固有名詞 / 曖昧なケース）→ `(none)` または補正なし:**
```
(none)
```

> `令利7年` → `令和7年`: 「令利」という元号は存在しないため高確信度で補正可。
> `administraion` → `administration`: 唯一の正解があるスペルミスのため補正可。
> `山回太郎` は `山田太郎` に補正しない: 「山回」という苗字が存在しうる固有名詞のため言語知識による補正は禁止。
...
チャット履歴が空の場合（ユーザー発言がない場合）は「Prior conversation」セクション自体を省略する。`chatHistoryToText()` がユーザーロールのメッセージを1件も含まない場合（初期グリーティングのみの状態）は空文字を返すため、セクション全体が出力されない。
...
*v0.3.2: MF-01c §2.1「画像確認必須」制約を緩和。`<corrected_text> must match exactly what appears in the image` と `Do not guess corrections for text that is unclear or obscured in the image` を削除し、「画像が不鮮明な場合でも高確信度なら言語知識で補正可・ただし固有名詞と曖昧なケースは禁止」を規定する 4 ルールに置き換え。§2.4 に補正可否の境界例を追加。*
