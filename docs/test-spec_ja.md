...
> バージョン: 0.2.0
> 作成日: 2026-05-07
> 最終更新: 2026-05-30（入力テキスト上限を 8,000 → 16,000 文字に修正、MF-01b/MF-01c テストケース追加 — TC-01-05, TC-01b-01〜03, TC-01c-01〜03, TC-02-11, TC-ERR-06）
...
### TC-01-05: 16,000 文字超過のトリム

| 項目 | 内容 |
|------|------|
| 前提 | 17,000 文字のテキストを含む PDF |
| 手順 | `TextExtractor.extract()` を呼ぶ |
| 期待結果 | 返り値が最大 16,000 文字である<br>先頭から 16,000 文字が保持されている |
...
### MF-01b: ImageTextExtractor テスト

### TC-01b-01: 画像からの正常 OCR

| 項目 | 内容 |
|------|------|
| 対象 | `ImageTextExtractor.extractFromBitmap()` |
| 種別 | インテグレーション（ML Kit テストライブラリ） |
| 前提 | テキストを含む Bitmap（テスト用画像）、sourceLanguage = `"ja"` |
| 手順 | `ImageTextExtractor.extractFromBitmap(bitmap, "ja")` を呼ぶ |
| 期待結果 | 空でない文字列が返る |

### TC-01b-02: OCR タイムアウト（30 秒）

| 項目 | 内容 |
|------|------|
| 対象 | `ImageTextExtractor.extractFromBitmap()` |
| 種別 | ユニット（`TestCoroutineScheduler`） |
| 手順 | ML Kit 処理をブロックしたまま仮想時間を 30,001 ms 進める |
| 期待結果 | `ExtractionError.OcrFailed` がスローされる |

### TC-01b-03: OCR 結果が空の場合

| 項目 | 内容 |
|------|------|
| 対象 | `ImageTextExtractor.extractFromBitmap()` |
| 種別 | ユニット（ML Kit モック） |
| 前提 | ML Kit が空テキストを返すようモック |
| 期待結果 | `ExtractionError.OcrFailed` がスローされる |

### MF-01c: OcrCorrector テスト

### TC-01c-01: OCR 補正の正常動作

| 項目 | 内容 |
|------|------|
| 対象 | `OcrCorrector.correct()` |
| 種別 | ユニット（`LlmModelHelper` モック） |
| 前提 | モックが `"CORRECT: 誤認識|正しいテキスト"` を含む文字列を返す |
| 手順 | `OcrCorrector.correct(model, "...誤認識...", listOf(bitmap))` を呼ぶ |
| 期待結果 | 返り値に `"正しいテキスト"` が含まれる |

### TC-01c-02: 推論失敗時は元テキストをそのまま返す
...
| 種別 | ユニット（`LlmModelHelper` モック） |
| 前提 | モックが例外をスロー |
| 期待結果 | `ocrText` がそのまま返る（例外がスローされない） |

### TC-01c-03: タイムアウト時は元テキストをそのまま返す

| 項目 | 内容 |
|------|------|
| 対象 | `OcrCorrector.correct()` |
| 種別 | ユニット（`TestCoroutineScheduler`） |
| 手順 | 仮想時間を 60,001 ms 進める |
| 期待結果 | `ocrText` がそのまま返る（クラッシュしない） |
...
### TC-02-11: MF-02 への入力テキストが 16,000 文字以内にトリムされる

| 項目 | 内容 |
|------|------|
| 種別 | ユニット（ViewModel） |
| 前提 | `TextExtractor` が 17,000 文字のテキストを返す |
| 期待結果 | `FieldExtractor.extract()` に渡されるテキストが 16,000 文字以内にトリムされている<br>（仕様書 §9.4: MF-02 入力上限 16,000 文字） |
...
### TC-ERR-06: テキストエリア直接入力が 16,000 文字超過

| 項目 | 内容 |
|------|------|
| 前提 | **テキストエリアに直接** 16,001 文字を貼り付ける（PDF 選択ではなくテキスト入力経由） |
| 期待結果 | 「書類が長すぎます（上限 16,000 文字）」が表示される<br>`startAnalysis()` が中断される |
| 備考 | PDF 選択時は仕様書 §7.1 の TextExtractor が自動で 16,000 文字にトリムするため本エラーは発生しない（TC-01-05 で確認済み）。本ケースはテキスト直接入力パスに限定する |
