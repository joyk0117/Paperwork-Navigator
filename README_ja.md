...
<img src="docs/screenshots/s02_review_screen.png" width="200" alt="S-02 レビュー画面" />

---

## 使い方

**1. インストール** — [最新APK](https://github.com/joyk0117/Paperwork-Navigator/releases/latest/download/app-debug.apk) をダウンロードして端末にインストールします。初回は「提供元不明のアプリ」の許可が必要な場合があります。

**2. モデルのダウンロード（初回のみ）** — アプリを起動すると Model Manager 画面が開きます。**Gemma 4 E2B**（高精度は **E4B**）を選択してダウンロードします。完了後はオフラインで動作します。

**3. タスクの選択** — ホーム画面で **Document Review** をタップします。

**4. 書類の入力** — 入力画面で PDF・カメラ・ギャラリー・他アプリからの共有のいずれかを選択して書類を読み込みます。

**5. 解析の実行** — 読み込み後、端末内で自動的に解析が始まります。完了するとレビュー画面に遷移します。

**6. レビュー画面で操作** — 期限・必要アクション・警告がカテゴリ別に表示されます。各項目から以下の操作を実行できます。
- **カレンダーに追加** — 期限をカレンダーアプリに登録
- **地図で見る** — 発行元住所を地図アプリで表示
- **翻訳** — 15 言語に翻訳して原文と並べて表示
- **チャット** — 書類内容について Gemma 4 に質問

**7. 問い合わせコンテキストの生成（任意）** — レビュー画面から「問い合わせコンテキスト生成」を開き、PII マスク済みのテキストを生成します。Android 共有シートで外部 AI や専門家に送信できます。
